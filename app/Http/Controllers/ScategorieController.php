...
class ScategorieController extends Controller
{
    ...
    {
        try{
            $scategorie = Scategorie::with("categorie")->get();//a3tini sous catégorie w a3tini joiture m3a categorie
    ...
    public function destroy($id)
    {
        try{
            $scategorie = Scategorie::findOrFail($id);
            $scategorie->delete();
            return response()->json("sous catégorie supprimé avec succées");
          }catch(\Exception $e){
              return response()->json($e->getMessage(),$e->getCode());
      }
    }
